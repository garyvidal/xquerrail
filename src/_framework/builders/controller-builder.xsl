...
{
  ( 
	    response:body(controller:update(request:request()))
	    response:flush()
  )
};
</xsl:template>
...
declare function controller:create() {
  let $create := controller:create(request:params())
  return
  (
...
{
      response:data("delete",controller:delete(request:flush())),
      response:flush()
};
 </xsl:template>
...
{
  response:body(controller:get(request:request())),
  respone:flush();
};
...
{
  response:data("search_results",controller)
};
 </xsl:template>
 <!--
  ============================================================
 ...
 <xsl:function name="util:get-primary-id">
   ...
   <xsl:value-of select="$controller/*[@id-field]/@name"/>
 </xsl:function>
