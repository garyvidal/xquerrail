...
$application-name as xs:string
   ...
   config:get-domain($application-name)
};
...
declare function domain:get-model-navigation($application-name,$model-name)
{
   let $model-nav := domain:get-domain-model($application-name,$model-name)/domain:navigation
   return
     if($model-nav) 
     then $model-nav
     else <domain:navigation newable="true" sortable="true"/>
};
