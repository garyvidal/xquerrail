...
declare function control-option($name,$type,$default,$values)
{
  ...
  <values>{for $v in $values 
   return 
    if($v instance of xs:anyAtomicType) then <option value="{$v}">{fn:data($v)}</option>
    else $v
  }</values> 
  </option>
};
...
let $multi := $options/occurence/text() = ("*","+")
let $model-name := fn:tokenize($options/reference,":")[2]
let $controller := domain:get-model-controller("application",$model-name)
let $model      := domain:get-domain-model("application",$model-name)
return
    <div class="referenceSelect">
           { form:render-before($name,$type,$value,$options) }
           <select id="{$controller/@name}" name="{$name}" class="field select large {if($multi) then 'multiple' else ()}" >
            { 
               if($options/occurence/text() = ("*","+")) 
               then attribute multiple { "multiple" }
               else (),
               if(fn:not($multi)) then 
               ...
               var selected = [
                   {
                     ...
                     return ( 
                         (:fn:concat('"',fn:data($node/@ref-id),'"'),:)
                           fn:concat('"',$node,'"'),
                         if($pos = fn:count($value)) then () else ","
                     )
                   }
               ]
               populateRelatedOptions("{fn:data($controller/@name)}", selected, "{$options/occurence/text()}");
           </script>
           { form:render-after($name,$type,$value,$options) }
    </div>
};
