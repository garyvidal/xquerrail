...
module namespace form = "http://condenast.com/dam/2.0/lib/formbuilder";
  ...
  return
    typeswitch($qtype)
     ...
     (: Complex Element :)
      ...
      case element(grid) return form:build-kendo-grid($field,$value)
...
declare function form:build-kendo-grid($field,$value) {
	...
	fn:concat("[ ", 
	   fn:string-join(
     	   ...
     	   let $type := fn:data($item/@edittpe)
     	   return
                fn:concat('{ field: "', $name, '", title: "', $label, '" }')
	   , ",")
	, " ]")
	:)
	let $columnSchema := 
     ...
     form:render-before($field), 
      <div class="complexGridWrapper">
           ...
           <script type="text/javascript">
                //alert(kendo.stringify([{$fieldSchema}, {$columnSchema}]))
                buildKendoEditGrid('{$fieldKey}', {$modelData}, {$fieldSchema}, {$columnSchema}) 
           </script>
       </div>,
      form:render-after($field)
   )
...
declare function form:render-reference($field,$value) {
    ...
    <div class="referenceSelect">
           ...
           <select id="{fn:data($field/@name)}SelectBox" name="{form:get-field-name($field)}">
            {
                if($field/domain:constraint/@required = 'true')
