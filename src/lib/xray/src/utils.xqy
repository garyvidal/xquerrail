...
declare private variable $path-seperator := 
   if( xdmp:platform()  = "winnt") 
   then "\"
   else "/";   

declare private variable $path-regex := 
   if( xdmp:platform()  = "winnt") 
   then "\\"
   else "/";
...
declare function utils:get-functions($module-path as xs:string) as xdmp:function*
{
  ...
  return 
    ...
    return xdmp:function($qname, fn:replace($module-path, fn:replace(xdmp:modules-root(),"\\","\\\\"), $path-regex))
};


declare function utils:get-modules($test-dir as xs:string, $pattern as xs:string?) as xs:string*
{
  let $fs-dir := fn:concat(xdmp:modules-root(), fn:replace($test-dir, $path-regex, ""))
...
{
  fn:replace($path, fn:replace(xdmp:modules-root(),"\\","\\\\"), $path-regex)
};
