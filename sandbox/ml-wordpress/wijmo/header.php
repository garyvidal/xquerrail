<?php
/**
 * The Header for our theme.
 *
 * Displays all of the <head> section and everything up till <div id="main">
 *
 * @package WordPress
 * @subpackage Wijmo
 * @since Wijmo 1.0
 */
?>
<!DOCTYPE html>
<html <?php language_attributes(); ?>><head>
<meta charset="<?php bloginfo( 'charset' ); ?>" />
<title>
<?php
	/*
	 * Print the <title> tag based on what is being viewed.
	 */
	global $page, $paged;

	wp_title( '|', true, 'right' );

	// Add the blog name.
	bloginfo( 'name' );

	// Add the blog description for the home/front page.
	$site_description = get_bloginfo( 'description', 'display' );
	if ( $site_description && ( is_home() || is_front_page() ) )
		echo " | $site_description";

	// Add a page number if necessary:
	if ( $paged >= 2 || $page >= 2 )
		echo ' | ' . sprintf( __( 'Page %s', 'twentyten' ), max( $paged, $page ) );

	?>
</title>
<link rel="profile" href="http://gmpg.org/xfn/11" />
<!-- Styleesheets -->
<link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo( 'template_directory' ); ?>/css/midnight/jquery-ui-1.8.6.custom.css" />
<link href="http://cdn.wijmo.com/jquery.wijmo-open.1.1.4.css" rel="stylesheet" type="text/css" />
<script src="http://ajax.microsoft.com/ajax/jquery/jquery-1.5.1.min.js" type="text/javascript"></script>
<script src="http://ajax.aspnetcdn.com/ajax/jquery.ui/1.8.11/jquery-ui.min.js" type="text/javascript"></script>
<script src="http://cdn.wijmo.com/external/jquery.glob.min.js" type="text/javascript"></script>
<script src="http://cdn.wijmo.com/external/jquery.mousewheel.min.js" type="text/javascript"></script>
<script src="http://cdn.wijmo.com/external/raphael.js" type="text/javascript"></script>
<script src="http://cdn.wijmo.com/jquery.wijmo-open.1.1.4.min.js" type="text/javascript"></script>

<!-- Initialization Script -->
<script type="text/javascript" src="<?php bloginfo( 'template_directory' ); ?>/js/init.js"></script>


<link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo( 'stylesheet_url' ); ?>" />

<!-- Wijmo Widget styleesheet and Theme -->
<link rel="pingback" href="<?php bloginfo( 'pingback_url' ); ?>" />
<?php
	/* We add some JavaScript to pages with the comment form
	 * to support sites with threaded comments (when in use).
	 */
	if ( is_singular() && get_option( 'thread_comments' ) )
		wp_enqueue_script( 'comment-reply' );

	/* Always have wp_head() just before the closing </head>
	 * tag of your theme, or you will break many plugins, which
	 * generally use this hook to add elements to <head> such
	 * as styles, scripts, and meta tags.
	 */
	wp_head();
?>
</head>
<body <?php body_class(); ?>>
<div class="hfeed">
<div id="header" class="ui-widget ui-widget-content">

<!--<script type="text/javascript"
  src="http://jqueryui.com/themeroller/themeswitchertool/">
</script>
<div id="switcher-container" class="ui-corner-left">
<div id="switcher-trigger"><span class="ui-icon ui-icon-arrowthickstop-1-w"></span></div>
<div id="switcher"></div>
</div>-->

  <div id="topbar" class="ui-widget-header">
    <div class="wrapper">
      <?php $heading_tag = ( is_home() || is_front_page() ) ? 'h1' : 'div'; ?>
      <<?php echo $heading_tag; ?> id="site-title"> <span> <a href="<?php echo home_url( '/' ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home">
      <?php bloginfo( 'name' ); ?>
      </a> </span> </<?php echo $heading_tag; ?>>
      <!-- Wijmo Login Dialog -->
      <?php if ( is_user_logged_in() ) : ?>
      <div class="header-meta">
        <div id="wijlogout"> <a href="<?php echo wp_logout_url( $redirect ); ?>">Log Out</a> </div>
        <?php wp_register('<div id="register" class="button">','</div>'); ?>
        <?php get_search_form(); ?>
      </div>
      <?php else : ?>
      <div class="header-meta">
        <div id="wijlogin"> <a href="#" onClick="$('#wijmodialog').wijdialog('open')">Log In</a> </div>
        <?php wp_register('<div id="register" class="button">','</div>'); ?>
      </div>
      <?php endif;?>
      <div id="wijmodialog" title="Log In">
        <?php wp_login_form('redirect=index.php'); ?>
      </div>
    </div>
  </div>
  <div class="wrapper">
    <div id="masthead">
      <div id="branding" role="banner">
        <!-- Navigation -->
        <div id="access" role="navigation">
          <?php /*  Allow screen readers / text browsers to skip the navigation menu and get right to the good stuff */ ?>
          <div class="skip-link screen-reader-text"><a href="#content" title="<?php esc_attr_e( 'Skip to content', 'twentyten' ); ?>">
            <?php _e( 'Skip to content', 'twentyten' ); ?>
            </a></div>
          <?php /* Our navigation menu.  If one isn't filled out, wp_nav_menu falls back to wp_page_menu.  The menu assiged to the primary position is the one used.  If none is assigned, the menu with the lowest ID is used.  */ ?>
          <ul id="wijmomenu">
            <?php wp_list_pages( 'title_li=' ); ?>
          </ul>
        </div>
        <!-- Wijmo Super Panel Carousel -->
        <div id="bannershadow">
          <div id="wijmosuperpanel">
            <div class="carousel"> <img src="<?php bloginfo( 'template_directory' ); ?>/images/headers/berries.jpg" alt="Berries" />
			<img src="<?php bloginfo( 'template_directory' ); ?>/images/headers/cherryblossoms.jpg" alt="Cherry Blossoms" /> 
			<img src="<?php bloginfo( 'template_directory' ); ?>/images/headers/concave.jpg" alt="Concave" /> 
			<img src="<?php bloginfo( 'template_directory' ); ?>/images/headers/fern.jpg" alt="Fern" />
			</div>
          </div>
        </div>
        <!-- #branding -->
        <!-- #access -->
      </div>
      <!-- #masthead -->
    </div>
  </div>
</div>
<!-- #header -->
<div id="main">
